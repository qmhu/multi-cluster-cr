...
//var newMCController = mccontroller.New
...
func NewControllerManagedBy(m mcmanager.Manager) *Builder {
...
func (blder *Builder) bind(r reconcile.Reconciler) error {
	blder.r = r
	blder.mgr.GetMultiClusterController().AddBuilder(blder)
	return nil
}

func (blder *Builder) InjectCluster(cluster mcsource.Cluster) {
	blder.cluster = cluster
}

func (blder *Builder) Construct() (controller.Controller, error) {
	if blder.r == nil {
	...
	if err := blder.doControllerWithCluster(blder.r); err != nil {
		return nil, err
	}
	// Set the Watch
...
func (blder *Builder) doControllerWithCluster(r reconcile.Reconciler) error {
	...
	manager, err := blder.mgr.GetSingeClusterManager(blder.cluster)
	if err != nil {
		return err
	}

	// Build the controller and return.
	blder.ctrl, err = newController(blder.getControllerName(gvk), manager, ctrlOptions)
	wrappedController := mcmanager.Controller{
		Controller: blder.ctrl,
		Name: blder.cluster.Name,
	}
	return blder.mgr.Add(wrappedController)
}
